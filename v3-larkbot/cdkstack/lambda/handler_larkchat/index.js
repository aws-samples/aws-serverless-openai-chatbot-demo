...
const refuse_strategies = process.env.refuse_strategy.split('|');
const refuse_answers = process.env.refuse_answer.split('|');
...
const initLarkClients = () => {
  ...
  let refuse_strategies_map = {};
  let refuse_answers_map = {};
  for (let i = 0; i < appIds.length; i++) {
    ...
    refuse_strategies_map = {...refuse_strategies_map,[appIds[i]]:refuse_strategies[i]};
    refuse_answers_map = {...refuse_answers_map,[appIds[i]]:refuse_answers[i]};

  }
  return { lark_clients_map, lark_config_map, lark_id2sec_map, lark_tenants_map,system_roles_map,system_prompts_map,refuse_strategies_map,refuse_answers_map};
}
...
export const handler = async (event) => {
  ...
  const { lark_clients_map, lark_config_map, lark_id2sec_map, lark_tenants_map,system_roles_map,system_prompts_map ,refuse_strategies_map,refuse_answers_map} = initLarkClients();
  ...
  const refuse_strategy = refuse_strategies_map[app_id] === undefined?'':refuse_strategies_map[app_id];
  const refuse_answer = refuse_answers_map[app_id] === undefined?'':refuse_answers_map[app_id];
  console.log(`app id:${app_id} system_role:${system_role}, system_prompt:${system_prompt},refuse_strategy:${refuse_strategy}`);
  ...
  const payload = {
    ...
    refuse_strategy:refuse_strategy,
